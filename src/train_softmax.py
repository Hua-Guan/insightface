...
import pickle
import numpy as np
from src.image_iter import FaceImageIter
from src.image_iter import FaceImageIterList
import mxnet as mx
...
from src.symbols import fresnet
from src.symbols import finception_resnet_v2
from src.symbols import fmobilenet
from src.symbols import fmobilenetv2
from src.symbols import fmobilefacenet
from src.symbols import fxception
from src.symbols import fdensenet
from src.symbols import fdpn
from src.symbols import fnasnet
from src.symbols import spherenet
from src.eval import verification # 模型验证
import sklearn
...
def parse_args():
  ...
  parser.add_argument('--data-dir', default='./data/lfw_data', help='training set directory')
  parser.add_argument('--prefix', default='./models/train_self', help='directory to save model.')
  parser.add_argument('--pretrained', default='', help='pretrained model to load') # 预训练模型
  ...
  parser.add_argument('--loss-type', type=int, default=4, help='loss type,1-softmax,2-SphereFace,3-cosineface,4-arcface,5-gezhongjiehe') #使用loss的种类
  parser.add_argument('--verbose', type=int, default=100, help='do verification testing and model saving every verbose batches')
  ...
  parser.add_argument('--emb-size', type=int, default=512, help='embedding length') # 特征向量维度
  parser.add_argument('--per-batch-size', type=int, default=4, help='batch size in each context') # batch-size
  ...
  parser.add_argument('--target', type=str, default='lfw', help='verification targets') # 验证集数据
...
def train_net(args):
    ...
    """
    ctx = [mx.gpu()]
    prefix = args.prefix  #model_path
    prefix_dir = os.path.dirname(prefix)
    if not os.path.exists(prefix_dir):
      os.makedirs(prefix_dir)
    end_epoch = args.end_epoch
    args.ctx_num = len(ctx)
    args.num_layers = int(args.network[1:]) # 50
    ...
    data_dir_list = args.data_dir.split(',') # data_dir
    ...
    path_imgrec = os.path.join(data_dir, "train.rec")  # 训练集
